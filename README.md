...
├── arm_clip/
│   ├── arm_clip.FCStd             # アームクリップ（蛇腹バンド＋両端クリップ一体）
│   ├── arm_clip.stl               # 3Dプリント用STL（本体＋レバー2個）
│   └── arm_clip.step              # STEP（CAD交換用）
└── fake_green_planter/
    ├── fake_green_planter.FCStd   # 壁掛けフェイクグリーン用プランター
    ├── fake_green_planter.stl     # 3Dプリント用STL（直立向き・サポート不要）
    ├── isometric.png              # レンダリング画像
    └── README.md                  # 仕様・印刷設定・技術メモ
```
...
`Part.makeFillet()` は、スプライン押し出し（BSpline断面のextrude）やスプラインとの
ブーリアン融合体に対してはほぼ通らず、`StdFail_NotDone` かGUIタイムアウトになる。
角を丸めたい形状は「フィレットで後から丸める」のではなく、**最初から丸く作る**のが確実。

- **一定断面の帯状パーツ**（例: 蛇腹バンド）→ 角R付きの断面ワイヤを中心線に沿って
  `MakePipeShell` でスイープする。生成直後に `isValid()==False` でも `Shape.fix()` で
  修復できることが多い。
- **断面が変化する接続部**（例: 細い帯 → 太い壁のガセット）→ 角R付きの丸角矩形を
  2断面用意して `Part.makeLoft(..., ruled=False)` で滑らかに補間する。
  角張ったガセット（箱から円柱をカット）より自然な「トランペット状」の接続になる。
- クリップ本体のような**角柱＋円柱だけの単純形状**は、融合前のプリズム段階で
  `makeFillet` を掛ければ成功する。丸めたいエッジは、鋭角のうちに個別指定し、
  スプライン側の面へ接線伝播しないよう順序を工夫する（外周コーナーは最後に掛ける）。
...
### アームクリップ（袖上げバンド・全部PLA一体造形）

市販のワイシャツ袖上げバンド（金属クリップ＋ゴム帯）の写真を添付し、
「両端のクリップは一体成型」「フィラメントはPLA」という条件で造形した事例。
金属バネやゴムを使わず、**蛇腹バンド＋レバー式クリップを1ファイルで一体印刷**する方針を、
対話的に詰めてから設計した。

```
添付の写真のようなアームクリップを３Dプリンタで作りたい。
両端のクリップは一体成型したい。３DプリンタのフィラメントはPLA
```
...
| 項目 | 決定 |
|---|---|
| 全体構成 | 全部PLA（蛇腹バンドで疑似的に伸縮） |
| クリップ機構 | プリントインプレースのレバー式（偏心カム） |
| 対象生地 | 薄手のワイシャツ生地 |
| バンド | 蛇腹（窓関数付き正弦波）・全長120mm・幅12mm・板厚0.8mm |
| クリップ | 幅12mm・レバー開で挟み口2.84mm / 閉で0.5mm |
| ヒンジ | ピンφ3 / 穴φ3.7（クリアランス0.35mm）で一体印刷 |
| 造形ベッド | Bambu A1（256mm角） |

#### 蛇腹バンドのFEM検証（PLA・E=3.3GPa）

板厚・振幅・山数を振ってCalculiXで剛性を比較し、実用的な伸び代と応力のバランスを取った。

| 板厚 | 振幅 | 山数 | 幅 | 剛性 | 5N時応力 | 判定 |
|---|---|---|---|---|---|---|
| 1.2 | ±4 | 6 | 15 | 5.0 N/mm | 1.5 MPa | 硬すぎ |
| 0.8 | ±7 | 5 | 15 | 1.6 N/mm | 6.6 MPa | △ |
| **0.8** | **±10** | **4** | **12** | **0.83 N/mm** | **10.6 MPa** | **採用** |

> 採用案は10mm伸ばして約8N・応力約18MPa。PLA降伏（50〜60MPa）に対し余裕があり、
> 繰り返し使用の目安（<25MPa）にも収まる。実用的な伸び代は8〜10mm。

#### レバー式クリップ（プリントインプレース）

- 偏心カム（R6・偏心2mm）を回すとアンビルとの隙間が 2.84mm → 0.5mm に締まる。
  バンドの張力がカムを締める向きに働くよう回転方向を設定。
- カム外周に滑り止め溝。**溝はカム単体に切ってからアームを融合**しないと、
  アーム付け根まで溝が貫通してスロット状の穴になる（造形の順序が重要）。
- ヒンジはクリアランス0.35mmで印刷後にレバーをひねって固着を剥がす。サポート不要。

#### 印刷のヒント（Bambu A1 / PLA）

- 蛇腹が立った向き（Z=高さ12mm）のまま印刷。接地が細いのでブリム5mm以上。
- ウォール2周（0.8mm=2周ぴったり）、外壁は低速で蛇腹をきれいに。
- レバーのヒンジは自己ブリッジで造形、印刷後に可動化する。

> バンドと壁の接続や角の丸めで `makeFillet` が通らない問題は、
> [Tips「スプライン形状に絡む部分の角丸め」](#スプライン形状に絡む部分の角丸め)を参照。


---

### 壁掛けフェイクグリーン用プランター（ミドリエ easy pot 参照）

市販の壁掛け底面給水ポット（ミドリエデザイン easy pot）のAmazon商品ページを渡し、
「まだモデリングは開始しない」と伝えて設計相談から始めた事例。
用途確認の結果、**給水しない（フェイクグリーン用）ため防水・二重構造を省略**し、
一体シェル＋キーホールスロットのシンプル構造に落とし込んだ。

```
このサイトのプランターに似たものをつくって３Dプリンタでプリントアウトしたい．
まだ，モデリングは開始しない．「https://www.amazon.co.jp/...」
```
...
| 項目 | 決定 |
|---|---|
| 用途 | フェイクグリーン用（給水なし → 防水不要） |
| 全体 | 幅120 × 高さ115 × 奥行き105mm、D字（半楕円）断面 |
| テーパー | 底面90%スケール（約3°）、上縁フィレットR1.0 |
| 壁厚 | 2.4mm、底閉塞（水抜き穴なし） |
| 壁固定 | 背面キーホールスロット（φ9大径部 / 幅4.5 × 長さ12mm、上縁から20mm） |
| 内底 | 十字リブ 60 × 2 × 高さ15mm（フォーム・茎の固定用） |

キーホール周囲は背面壁を局所4mm厚に増し、外側ウェブ2mm＋内側ヘッドチャネルで
ネジ頭がスライドして掛かる標準構造とした（寸法は絶対値・スケール変更時も維持）。

#### 印刷のヒント（Bambu A1 / PLA）

- **直立（底面接地）のまま印刷、サポート不要**。当初は背面を下にする案だったが、
  前面の楕円クレストが水平オーバーハングになり化粧面にサポート跡が付くため、
  直立に変更した（壁は3°テーパーでほぼ垂直、キーホールは垂直壁の貫通穴）。
- ウォール2〜3周、インフィル10〜15%で十分（給水なし・軽量用途のため）。

#### 技術メモ: ロフトの対応ずれによる形状膨張

別々に構築した半楕円ワイヤ同士を `Part.makeLoft` すると対応点がずれ、
前面が設計値105mmに対して117.6mmまでオーバーシュートした（`ruled=True` でも解消せず）。

**解決策: 同一トポロジのワイヤをロフトする。**
上面プロファイルを1つだけ構築し、下面は `transformGeometry` による縮小コピーで作ると、
エッジ構成・パラメータ化が完全に一致するため対応ずれが起きない。

```python
def scaled_copy(wire, s, yc):
    """(0, yc) を中心にXY一様スケール（トポロジ保存）"""
    m1 = App.Matrix(); m1.move(App.Vector(0, -yc, 0))
    m2 = App.Matrix(); m2.scale(s, s, 1.0)
    m3 = App.Matrix(); m3.move(App.Vector(0, yc, 0))
    return wire.transformGeometry(m3.multiply(m2).multiply(m1))

w_bot = scaled_copy(w_top, 0.90, 0.0)
outer = Part.makeLoft([w_bot_at_z0, w_top_at_z115], True, True)
```

- 半楕円プロファイルは `Part.ArcOfEllipse` ＋ 閉じ線で直接構築し、
  背面のブーリアンカット（膨張の別要因）も回避した。
- 壁厚2.4mmの上縁にフィレットR2は形状不成立（ほぼ全周丸めになるため）。R1.0が実質上限。

![fake green planter](fake_green_planter/isometric.png)
